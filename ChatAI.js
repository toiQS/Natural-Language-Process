...
class ChatAI {
    ...
    getMessage() {
        ...
        Sentence.forEach(element => {
            if (element.includes("xin")) {
                return resultOfSentence = false;
            }
        });
        // console.log(resultOfSentence);
        if (resultOfSentence == true) {
            ...
            this.selectedConversation.messages.push({
                role: 'assistant',
                // content: data.choices[0].message.content,
                content: "Câu từ có chứa từ nhạy cảm trong chính sách quy đinh của chương trình.",
                date: new Date(),
                // total_tokens: data.usage.total_tokens,
                // prompt_tokens: data.usage.prompt_tokens,
                // completion_tokens: data.usage.completion_tokens
    ...
    saveSettings() {
        localStorage.setItem('settings', JSON.stringify({
            api_key: this.APIKey, max_tokens: this.maxTokens,
